...
export default function EditorScreen ({ route, navigation }) {
  ...
  function saveNote () {
    ...
    axios({
      ...
      url: 'http://192.168.0.103:1337/api/notes' + (route.params && route.params.note ? '/' + route.params.note.id : ''),
      data: {
        data: {
          title,
          content,
  ...
  function deleteNote () {
    axios.delete('http://192.168.0.103:1337/api/notes/' + route.params.note.id)
      .then(() => {
        //redirect back to home screen
      navigation.goBack();
      })
